...
const updateForDate = (event: DragEvent) => {
    ...
    if (task && task.date !== dateProps.value.toLocaleDateString()) {
        if (confirm(`¿Desea mover la tarea "${task.title}" a la fecha ${transformStringToDate(props.fecha)}?`)) {
            console.log('si')
        }
...
<template>
    <div>
        ...
        <div class="flex flex-col justify-start items-center h-20 " @dragover="$event.preventDefault()"
            @drop="updateForDate">
